...
- YouTube Music's own unofficial `ytmusicapi`-based playlist API was originally used for the write path (first via cookie auth known to degrade after hours of heavy use, then an attempted OAuth-token reuse). KTD4's 2026-08-06 supersession replaces it entirely with the official Data API v3's `playlists`/`playlistItems` endpoints, which are fully supported for this OAuth client. This removes the unofficial-API risk from the write path altogether — both paths now depend only on the official, versioned Data API, whose own risk surface is standard OAuth-credential health (KTD6's health-state/reconnect mechanism still applies to that).
...
- KTD4. **Server-side web OAuth for the official Data API, used for both the detection path and the write path — no `ytmusicapi`, no cookie auth** (session-settled: user-directed — chosen over keeping both mechanisms CLI-only, or fixing only the OAuth detection path: a persistent backend and a remote mobile client can't rely on a local-server browser popup, and "stays organized automatically" requires an in-app reconnect path rather than a CLI chore). **Superseded twice on 2026-08-06:**
  1. First pass: live testing hit a real `browser.json` cookie expiry (401) on the write path. `YouTubeDataApiClient`'s scope was widened from `.readonly` to full `youtube`, and the write path was pointed at `ytmusicapi`'s `OAUTH_CUSTOM_FULL` auth type (which accepts a caller-supplied `Authorization: Bearer <token>` header dict verbatim, no refresh/rewriting of its own) so it could reuse that same OAuth token instead of a cookie file.
  2. That reuse turned out not to work: `music.youtube.com`'s internal API rejected the Bearer token outright with HTTP 400, on plain reads as well as writes, while the identical token succeeded against the official Data API. This matches `ytmusicapi`'s own OAuth support only ever working with a Google-registered "TVs and Limited Input devices" client type — a type that only supports the device-code grant, the same grant already confirmed broken upstream (see the Stop Conditions in the Goal Capsule). There is no browser-redirect-flow token that `ytmusicapi`'s internal API will accept.
  3. Final shape: `ytmusicapi` was dropped entirely. `YouTubeDataApiClient` now implements the write path directly against the official Data API's `playlists.insert`/`playlistItems.insert` endpoints, using the exact same widened-scope OAuth credential as detection. `YouTubeDataApiClient` is now the sole `MusicServiceClient` implementation (`ytmusic_client.py` and the `ytmusicapi` dependency are both removed). Anyone with an existing `.readonly`-scoped token on file must re-authorize once via `/api/v1/auth/youtube/authorize` to pick up the wider scope.
...
- KTD17. **Ingestion/detection health and write health are two independently surfaced states, never blended into one signal** — a stalled detection path and a degraded write path need distinguishable reconnect actions, and hard OAuth-token revocation (not just expiry) gets its own manual-reconnect affordance rather than relying solely on refresh. Since KTD4's 2026-08-06 supersession both paths share one OAuth credential, but each path still independently sets its own status from its own call outcome — a shared root cause can fail both at once, but neither ever reports the other's state. Extends KTD6. Governs R10.
